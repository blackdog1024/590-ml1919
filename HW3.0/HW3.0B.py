...
# ANN PARAM
layers = ['overwrite', 5, 5, 5, 1]  # FIRST NUMBER OVERWRITTEN LATER BY INPUT SHAPE
...
def NN_eval(x,submatrices):
    for i in range(0, len(layers) - 2):
        print("submatrices[0].T.shape",submatrices[0].T.shape)
        print("x.shape",x.shape)
        print("submatrices[1].T.shape",submatrices[1].T.shape)
        x =  np.matmul(x,submatrices[0].T) + submatrices[1].T
        x = 1/(1+np.exp(-x))
    y = np.matmul(x,submatrices[0].T) + submatrices[1].T
    return y
